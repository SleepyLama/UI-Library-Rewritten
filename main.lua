...
function library:CreateWindow(args)
    ...
    -- name, version, icon
    task.spawn(function()
        if not isfile(string.format("%s",args.ConfigFolder)) then
            makefolder(string.format("%s",args.ConfigFolder))
        end
    end)
	name = args.Name or "Name"
	version = args.Version or "Version"
	icon = args.Icon or math.random()
	...
	function tabs:CreateTab(name)
		...
		function mytabbuttons:CreateFrame(name)
			...
			function pagebuttons:CreateButton(args)
                -- name, desc, callback
			...
			function pagebuttons:CreateLabel(args)
                ...
                pcall(function()
                    library.Flags[args.Flag] = {
                        ...
                        Type = "Label",
                        Set = function(val)
                            UpdateLabel2:Set(val)
                        end
                    }
                end)
                -- task.spawn(function()
                --     local Config = readfile(ConfigFolder.."/"..game.GameId..".txt")
                --     local Data = game:GetService("HttpService"):JSONDecode(Config)
                --     if Data[args.Flag] then
                --         UpdateLabel2:UpdateLabel() 
                --     end
                -- end)
				return UpdateLabel2
			end

			function pagebuttons:CreateSlider(args)
                -- name,min,max,callback
				name = name or "Slider"
				args.Min = args.Min or 16
				...
				function library4:SetValue(input)
					...
					local value = math.floor(( ((pos.X.Scale * args.Max) / args.Max) * (args.Max - args.Min) + args.Min ) * 100) / 100
					ValueText.Text = tostring(mr(value))
					library4["Value"] = value
					spawn(function() args.Callback(value) wait() ValueText.Text = mr(value) end)
				end;
			...
			function pagebuttons:CreateBox(args)
				args.Name = args.Name or "Input Text Here..."
				if args.Icon == "Default" then
					args.Icon = 10045753138
				else
					args.Icon = args.Icon
				end
				args.Callback = args.Callback or function() end
				...
				TextInput.PlaceholderText = args.Name --"Input Text Here..."
				TextInput.Text = ""
				...
				TextInput.FocusLost:Connect(function()
                    library.Flags[args.Flag].Value = TextInput.Text
					args.Callback(TextInput.Text)
				end)
                ...
                --     local Config = readfile(ConfigFolder.."/"..game.GameId..".txt")
                --     local Data = game:GetService("HttpService"):JSONDecode(Config)
                --     TextInput.Text = tostring(Data[args.Flag])
                --     args.Callback(TextInput.Text)
                -- end)
				return UpdateBox
			end

			function pagebuttons:CreateBind(args)
				local name = args.Name or "Keybind"
				local defaultkey = args.Default or "Unknown"
					...
					if keybindtoggle == false then
						...
						Thing = game:GetService("UserInputService").InputBegan:Connect(function(Key, IsNotFocused)
							...
							if Key.UserInputType == Enum.UserInputType.Keyboard then
								...
								args.Callback(Key.KeyCode.Name)
			...
			function pagebuttons:CreateToggle(args)
				args.Name = args.Name or "Title"
				args.Description = args.Description or "Description"
				args.Callback = args.Callback or function() end
				local UpdateToggle = {}
                ...
                function UpdateToggle:Set(bool)
                    if bool == true then
                        f = true
                        TS:Create(Dot,TweenInfo.new(.1),{BackgroundTransparency=0}):Play()
						spawn(function()
                ...
                --     local Data = game:GetService("HttpService"):JSONDecode(Config)
                --     if Data[args.Flag] then
                --         UpdateToggle:Set(Data[args.Flag]) 
                --     end
                -- end)
				return UpdateToggle
			end
    ...
    function tabs:SaveConfig()
		...
		writefile(args.ConfigFolder .. "/" .. game.GameId .. ".txt", tostring(game:GetService("HttpService"):JSONEncode(Data)))
    end
	return tabs
end
